...
module.exports.bodyGenerator = function (level, role, room = undefined, creepInfo = undefined) {
    ...
    try{    energyAmount = room.energyAvailable;}
    catch{log.a('error in module.body.generator getting energyAvailable')}
    ...
    switch (role) {
        ...
        case 'drone':
            ...
            break;
        case 'pioneer':  
     //   log.a('case pioneer in boidygen')
        energyScaling = false;
        work = _.floor((energyAmount * 0.4) / BODYPART_COST[WORK]) || 1;
        if (work > 15) work = 15;
        carry = _.floor((energyAmount * 0.1) / BODYPART_COST[CARRY]) || 1;
        ...
        case 'claimer':
            claim = 1;
            move = _.floor(energyAmount * .9 - BODYPART_COST[CLAIM]) || 1;
            if (move < 1) move = 1;
            if (move > 5) move = 5;
            break;
        case 'reserver':
