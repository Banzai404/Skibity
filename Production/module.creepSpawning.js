...
module.exports.processBuildQueue = function (room) {
    ...
    for (let availableSpawn of availableSpawns) {
        ...
        for (let topPriority of queue) {
            ...
            break;}
            catch {log.a('error in module.creep.spawning 49')}
        }
        ...
        if (queuedBuild) {
            ...
            if (typeof queuedBuild === 'object') {
                ...
                let name = role.slice(0, 6) + ' ' + room.name + ' ' + room.energyAvailable + '-' + getRandomInt(100, 999);
                ...
                switch (availableSpawn.spawnCreep(body, name, {
                    ...
                    case OK:
                        ...
                        if (!queuedBuild.operation) log.a(availableSpawn.room.name + ' Spawning a ' + role);
                        let roomQueue = {};
                        let globalQueue = {};
...
module.exports.essentialCreepQueue = function (room) {
    ...
    if (room.level < 7 && room.level === room.controller.level && !reboot) {
     //   console.log('let reboot')
        let container = Game.getObjectById(room.memory.controllerContainer);
   //     console.log(container)
   //     console.log(container.store)
        if (container) {
   //         console.log('if continer is true')
      //      if (container.store[RESOURCE_ENERGY] > CONTAINER_CAPACITY * 0.5) {
      //          console.log( room + ' ' + container.store[RESOURCE_ENERGY]  + ' / ' + CONTAINER_CAPACITY)
                
                number = ((container.store[RESOURCE_ENERGY] / CONTAINER_CAPACITY ) * (20 - room.level) + 2); //console.log('10 * at mod.creeepSpawning 184 number is ' + number);
                if (number > container.pos.countOpenTerrainAround()) {
                    number = container.pos.countOpenTerrainAround() + 5;
                    
                }
       //     }
        } else number = 20 - room.level;
    }
    ...
    if (!room.nukes.length && !INTEL[room.name].threatLevel) {
        ...
        if (level >= MAX_LEVEL - 1 && level >= 4) {
            ...
            if (room.memory.borderPatrol && !getCreepCount(undefined, 'longbow', room.memory.borderPatrol, 'borderPatrol')) {
                ...
                queueCreep(room, PRIORITIES.remHarvester, {
                    role: 'longbow',
...
module.exports.remoteCreepQueue = function (room) {
        ...
        for (let remoteName of JSON.parse(remoteRoomTargets[room.name])) {
            ...
            if (highestLevel && INTEL[remoteName].invaderCore) {
                ...
                if (!getCreepCount(undefined, 'attacker', remoteName)) {
                    queueCreep(room, PRIORITIES.remHarvester - 1, {
            ...
            if (highestLevel && INTEL[remoteName].threatLevel > 1) {
                if (INTEL[remoteName].tickDetected + CREEP_LIFE_TIME < Game.time) {
                    if (!getCreepCount(undefined, 'explorer', remoteName)) {
                        queueCreep(room, PRIORITIES.remHarvester - 1, {role: 'explorer', destination: remoteName})
                    }
            ...
            if (highestLevel) {
                ...
                if (INTEL[remoteName].sk && room.level >= 7) {
                    ...
                    if (getCreepCount(undefined, 'SKAttacker', remoteName)) {
                        if (getCreepCount(undefined, 'remHarvester', remoteName) < INTEL[remoteName].sources) {
                            queueCreep(room, PRIORITIES.remHarvester + (getCreepCount(undefined, 'remHarvester', remoteName) * 0.5), {
                                role: 'remHarvester', destination: remoteName
                            })
                        }
                        if (!getCreepCount(undefined, 'SKMineral', remoteName) && (!INTEL[remoteName].mineralCooldown || INTEL[remoteName].mineralCooldown < Game.time)) {
                            queueCreep(room, PRIORITIES.remHarvester, {role: 'SKMineral', destination: remoteName})
                        }
                    }
                } // Regular remotes
                    ...
                    if (INTEL[remoteName] && INTEL[remoteName].obstacles) {
                        if (!getCreepCount(undefined, 'cleaner', remoteName)) {
                            queueCreep(room, PRIORITIES.remHarvester, {role: 'cleaner', destination: remoteName})
                        }
                    }
                }
            }
            if (getCreepCount(undefined, 'remHarvester', remoteName)) {
                // Haulers
                // Get energy output for room
                if (Game.rooms[remoteName]) {
                    let harvesters = _.filter(Game.rooms[remoteName].myCreeps, (c) => c.memory.destination === remoteName && c.memory.role === 'remHarvester' && c.memory.setSourceAmount);
                    let energyOutput = 0;
                    ...
                    if (energyOutput / remoteRoomCount) {
                        let assignedHaulers = _.filter(Game.creeps, (c) => c.my && c.memory.overlord === room.name && c.memory.destination === remoteName && c.memory.role === 'remHauler');
                        let currentHaulingCapacity = 0;
                        if (assignedHaulers.length / 1.2) assignedHaulers.forEach((c) => currentHaulingCapacity += c.store.getCapacity());
                        // Spawn them if we don't meet the threshold
                        if (currentHaulingCapacity < energyOutput) {
                            queueCreep(room, PRIORITIES.remHauler + getCreepCount(room, "remHauler", remoteName), {
                                role: 'remHauler', destination: remoteName, misc: energyOutput
                            })
                        }
                    }
                }
            ...
            // Remote Road Builder
            if (getCreepCount(undefined, 'roadBuilder', room.name) < 7) {
    ...
    if (robberyTargets[room.name] && JSON.parse(robberyTargets[room.name]).length) {
        for (let remoteName of JSON.parse(robberyTargets[room.name])) {
            if (!INTEL[remoteName].hostile && !getCreepCount(undefined, 'remHauler', remoteName)) {
                queueCreep(room, PRIORITIES.remHauler, {role: 'remHauler', destination: remoteName})
...
module.exports.globalCreepQueue = function () {
    ...
    for (let key in operations) {
     ...
     //   let requestTime = operations[key].cached;
    //    log.a('operations[key].requeestTime ' + operations[key].requestTime)
        if (!priority) {
            if (INTEL[key]) {
        ...
        //priority is more important the longer it has been waiting.  adjust it by game.tick
     //   log.a('modify priority of global creeps in room queue...operations[key].priority/operations[key].requestTime ' + operations[key].priority + '/' + operations[key].requestTime)
     //   operations[key].priority = operations[key].priority - (Game.time - operations[key].cached) / 1000
        ...
        switch (operations[key].type) {
            ...
            // Rebuilding allies
            case 'rebuild':
          //      log.a('case rebuild in creep.spawn')

                try{
                    if (!INTEL[key] || !INTEL[key].threatLevel) {
         //               log.a('made it to 1')
                        if (getCreepCount(undefined, 'pioneer', key) < 15) {
                    //        log.a('made it to 2--attempting to queue pioneer room/priority/destination ' + 'room.name' + '/' + (PRIORITIES.pioneer + getCreepCount(undefined, 'pioneer', key)) + '/' + key)
                            //queueCreep(undefined, PRIORITIES.pioneer + getCreepCount(undefined, 'pioneer', key), {
                            queueCreep(undefined, PRIORITIES.pioneer + (getCreepCount(undefined, 'pioneer', key)/10), {
                                role: 'pioneer', 
                                destination: key
                            }, true);
                        }
               ...
               //         log.a('made it to 3')
                        if (INTEL[key].threatLevel && getCreepCount(undefined, 'longbow', key) < 2) {
                            log.a('made it to 4')
                    ...
                    log.a('error in module.creepSpawning.js...maybe intel?---trying catch')
                    if (getCreepCount(undefined, 'pioneer', key) < 8) {
                         queueCreep(undefined, PRIORITIES.pioneer + getCreepCount(undefined, 'pioneer', key), {
                            role: 'pioneer', destination: key
                        }, true);
            ...
            case 'robbery': // Middle room mineral mining
                if (!getCreepCount(undefined, 'remHauler', key)) {
                    queueCreep(undefined, priority, {
                        role: 'remHauler', destination: key, operation: 'robbery'
                    }, true)
                }
                break;
...
function displayQueue(room) {
    ...
    let roomQueue = {};
    if (room.memory.creepQueue) roomQueue = JSON.parse(room.memory.creepQueue);
  //  log.a('adding globalCreepQueue to the room queue...room.level/!INTEL[room.name].threatLevel/!importantBuilds ' + room.level + '/' + INTEL[room.name].threatLevel + '/' + importantBuilds)
  //  if (_.size(globalQueue) && room.level >= 3 && !INTEL[room.name].threatLevel && !importantBuilds) {
    if (_.size(globalQueue) && room.level >= 3 && !INTEL[room.name].threatLevel) {
    ...
    if (lower > 15) lower = 15;
    room.visual.rect(33, 0, 49, lower, {
        fill: '#ffffff', opacity: '0.55', stroke: 'black'
    });
    displayText(room, 34, 1, 'Creep Build Queue');
    let i = 0;
    if (_.size(queue)) {
        for (let item of queue) {
            ...
            displayText(room, 34, 2 + i, item.priority + ' ' + _.capitalize(item.role) + mil + ': ' + room.energyAvailable + '/' + cost + ' Age: ' + (Game.time - item.cached));
            i++;
        }
    }
    // Display spawning
    for (let spawn of activeSpawns) {
        ...
        displayText(room, 34, 2 + i, 'Spawning - ' + _.capitalize(spawningCreep.name.split("_")[0]) + ' - Ticks: ' + spawn.spawning.remainingTime);
        i++;
    }
    return queue;
}
