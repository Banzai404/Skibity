    ...
    if (!creep.memory.working) {
        ...
        if (!creep.memory.harvest && (creep.memory.energyDestination || creep.locateEnergy())) {
            ...
            if (source && (!INTEL[creep.room.name].owner || INTEL[creep.room.name].owner === MY_USERNAME) && (!INTEL[creep.room.name].reservation || INTEL[creep.room.name].reservation === MY_USERNAME)) {
                ...
                if (!spawn && !_.find(creep.room.myCreeps, (c) => c.id !== creep.id && c.memory.stationaryHarvester) && _.find(creep.room.myCreeps, (c) => c.id !== creep.id && c.memory.role === 'pioneer')) creep.memory.stationaryHarvester = true;
...
function hauling(creep) {
    ...
    let haulers = _.filter(creep.room.myCreeps, (c) => c.memory && ((c.memory.role === 'pioneer' && c.memory.task === 'haul') || c.memory.role === 'hauler' || c.memory.role === 'shuttle')).length < 1;
